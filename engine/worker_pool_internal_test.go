...
func (b *blockingDispatcher) Dispatch(_ TaskState, _ *OIDCConfig) int {
	n := b.active.Add(1)
	for {
		m := b.maxSeen.Load()
...
func TestDispatchConcurrencyIsBounded(t *testing.T) {
	...
	for range tasks {
		_, _, err := e.CreateTask(testParent, httpTaskState("", time.Time{}))
		require.NoError(t, err)
	}
...
func TestIdleQueuesDoNotLeakGoroutines(t *testing.T) {
	...
	const queues = 5
	for i := range queues {
		_, err := e.CreateQueue("projects/p/locations/l",
...
func TestDeleteAfterPauseDoesNotHang(t *testing.T) {
	...
	go func() { errc <- e.DeleteQueue(testParent) }()
