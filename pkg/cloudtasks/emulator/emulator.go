...
type ErrorQueueAlreadyExists struct{}

func (e ErrorQueueAlreadyExists) Error() string {
	return "queue already exists"
}

type ErrorQueueExistedBefore struct{}

func (e ErrorQueueExistedBefore) Error() string {
	return "queue existed before"
}

type ErrorQueueNotFound struct{}

func (e ErrorQueueNotFound) Error() string {
	return "queue not found"
}

type ErrorQueueDeleted struct{}

func (e ErrorQueueDeleted) Error() string {
...
// NewServer creates a new emulator server with its own queue bookkeeping
func NewEmulator(oidcTokenCreator OIDCTokenCreator, httpDoer HTTPDoer, resetOnPurge bool) *Emulator {
	return &Emulator{
		oidcTokenCreator: oidcTokenCreator,
		httpDoer:         httpDoer,
		queueRunners:     make(map[string]*queueRunner),
		options: serverOptions{
...
func (e *Emulator) GetQueues() []Queue {
	e.lock.RLock()
	defer e.lock.RUnlock()

	var queues []Queue
	for _, qr := range e.queueRunners {
		if qr != nil {
			queues = append(queues, qr.queue)
		}
	}
...
func (e *Emulator) GetQueue(name string) (Queue, error) {
	e.lock.RLock()
	qr, has := e.queueRunners[name]
	e.lock.RUnlock()

	if !has {
		return Queue{}, ErrorQueueNotFound{}
	}
	if qr == nil {
		return Queue{}, ErrorQueueDeleted{}
	}

	return qr.queue, nil
}

func (e *Emulator) CreateQueue(name string, config QueueConfig) (Queue, error) {
	e.lock.RLock()
	qr, has := e.queueRunners[name]
	e.lock.RUnlock()
	...
	if has {
		if qr != nil {
			return Queue{}, ErrorQueueAlreadyExists{}
		}
		return Queue{}, ErrorQueueExistedBefore{}
	}
...
func (e *Emulator) GetTasks(name string) ([]Task, error) {
	e.lock.RLock()
	qr, has := e.queueRunners[name]
	e.lock.RUnlock()
	...
	if qr == nil {
		return nil, ErrorQueueDeleted{}
	}

	return qr.getTasks(), nil
}

func (e *Emulator) addQueue(queue Queue) {
	e.lock.Lock()
	defer e.lock.Unlock()

	e.queueRunners[queue.Name] = newQueueRunner(e, queue)
}
