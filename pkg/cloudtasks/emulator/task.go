...
type HTTPRequest struct {
	...
	Headers         map[string]string
	Body            []byte
	MaybeOIDCToken  maybe.M[OIDCToken]
	MaybeOAuthToken maybe.M[OAuthToken]
}
...
type TaskConfig struct {
	MaybeAppengineHTTPRequest maybe.M[AppEngineHTTPRequest]
	MaybeHTTPRequest          maybe.M[HTTPRequest]
}
...
// taskRunner holds all internals for a task
...
// // When a task completes succesfully or runs out of attemots, it is removed and the cloud tasks API
// // returns a 404.
...
// 				log.Printf("Task %v ran out of attempts (%d), deleting", t.state.GetName(), retryConfig.GetMaxAttempts())
// 				t.Cancel()
// 			} else {
...
// // Cancel cancels the task if it is queued for execution. It informs the parent queue
// // that the task was cancelled so it can remove references to it.
// // It can be called:
// // - Directly as a user request
// // - From the task itself after it completes or runs out of attempts
// func (t *Task) Cancel(doCallback bool) {
// 	t.cancelMutex.Lock()
// 	defer t.cancelMutex.Unlock()

// 	// Already cancelled/deleted
// 	if t.cancel == nil {
// 		return
// 	}
